"""Regression tests for strict evaluation of identity-critical fields."""
...
def test_build_payload_raises_when_required_field_expression_is_invalid():
    engine = Engine()
    ctx = _make_ctx({"hostname": "leaf-01"})
    resolver = Resolver(ctx)

    with pytest.raises(ValueError, match="Required field 'name' failed"):
        engine._build_payload(
            [
                FieldConfig(name="name", value="undefined_func()"),
                FieldConfig(name="role", value="source('hostname')"),
            ],
            resolver=resolver,
            ctx=ctx,
...
def test_process_item_records_error_when_lookup_expression_is_invalid():
    engine = Engine()
    ctx = _make_ctx({"hostname": "leaf-01", "role": "leaf"})
